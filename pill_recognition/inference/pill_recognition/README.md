...
-> crop per pill
-> request recognizer selection
   - retrieval: AI Hub ResNet152 feature retrieval
   - aihub_classifier: AI Hub official 1000-class classifier
   - codeit: ZerofZero/codeit10_pj1 RTMDet + EfficientNet classifier
...
Codeit 데모 파이프라인을 기본값으로 쓰려면 `PILL_RECOGNIZER=codeit`와 `CODEIT_PILL_PROJECT_DIR`을 설정합니다.
...
Codeit recognizer 경로:

```bash
cd /home/gyuha_lee/pill/code/ai/pill_recognition/inference
source ../.venv/bin/activate
export PILL_RECOGNIZER=codeit
export CODEIT_PILL_PROJECT_DIR=/home/gyu/pill/external/codeit10_pj1
...
독립 API는 요청마다 recognizer를 바꿀 수 있습니다.
...
curl -X POST http://127.0.0.1:8001/recognize \
  -F "file=@sample.jpg" \
  -F "recognizer=retrieval"
```

지원값은 `codeit`, `retrieval`, `aihub_classifier`입니다. `GET /health`의 `request_recognizers`에서 현재 서버가 받는 값을 확인할 수 있습니다.
...
또는 인식 요청에 복약목록 식별자를 바로 넣을 수 있습니다. 이 경우 API가 내부에서 `allowed_pill_ids`로 변환한 뒤 retrieval search scope를 적용합니다.

```bash
curl -X POST http://127.0.0.1:8001/recognize \
  -F "file=@sample.jpg" \
  -F 'allowed_item_seqs=["198601052","199303108"]' \
  -F 'allowed_product_names=["대화와르파린나트륨정","타이레놀정500mg"]'
```

`allowed_item_seqs`와 `allowed_product_names`는 `/recognize`, `/crops/recognize`, `/crops/recognize-batch`에서 모두 사용할 수 있습니다. 응답의 `candidate_scope.input_scope_resolution`에는 입력된 item_seq/제품명 수, resolve 성공 개수, unresolved 목록이 포함됩니다.
...
# or search both original and foreground-normalized crops, then keep the best score
export PILL_RETRIEVAL_QUERY_PREPROCESS=multi_foreground
```
...
`multi_foreground`는 원본 crop과 전경 중심 crop을 둘 다 검색하고 index similarity가 더 높은 variant를 채택합니다. AIHub held-out crop 기본 성능을 최대한 유지하면서 배경이 많이 섞인 scene crop 후보 누락을 줄이기 위한 실험 옵션입니다. 서버 synthetic realistic v2 val 200장 기준으로 Top-3는 소폭 올랐지만 latency가 증가해 기본값으로 켜지 않습니다.

| Query preprocess | Detector F1 | Recognition Top-3 on matched | End-to-end Top-3 on GT | Mean total |
|---|---:|---:|---:|---:|
