...
```
이미지 → 전처리 → Gemini Vision → MFDS DB 매칭 → 성분 파싱 → 결과 반환
```
...
### 3단계 — MFDS DB 매칭 (`db.py`, `matcher.py`)

추출된 제품명으로 식약처 MFDS 데이터베이스(44,766건)에서 일치 제품을 찾습니다.

#### 검색 방식

1. **FULLTEXT 검색** — MySQL ngram parser (인덱스: `ft_product_name`)
2. **RapidFuzz 재랭킹** — `fuzz.partial_ratio`로 상위 10개 후보 재평가
3. **임계값 필터** — 유사도 70% 미만이면 미매칭으로 처리
...
Yeast, Mould, E. coli, S., Hexane, 헥산, 국문, 영문, ...
```
...
- AI 서버에서 `ingredients` 리스트를 반환하지만, Backend `/interactions` 엔드포인트와의 연동이 프론트엔드에서 아직 구현되지 않음
- 현재 프론트엔드는 목업 데이터 사용 중 (`API_BASE_URL = ''`)
...
- [ ] Backend 다중 성분 엔드포인트 연동
- [ ] 프론트엔드 `API_BASE_URL` 실제 서버 주소로 교체
