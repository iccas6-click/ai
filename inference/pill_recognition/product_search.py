...
from .aihub_classifier import AIHubProductInfo, load_aihub_product_master
...
def score_product(
    ...
    imprint = normalize_token(query.imprint)
    if imprint:
        product_imprints = {
            variant
        ...
        if any(value == imprint for value in product_imprints):
            score += 100
            reasons.append("각인 exact")
        elif any(imprint in value or value in imprint for value in product_imprints):
            score += 70
