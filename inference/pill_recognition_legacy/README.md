# Pill Recognition Legacy Baseline

여러 알약이 놓인 사진 한 장에서 알약 위치를 탐지하고, 알약별 Top-3 제품 후보를 반환하는 baseline입니다.

이 디렉터리는 v1 실험 파이프라인 보존용입니다. 기본 앱은 `inference/pill_recognition/`의 provider 기반 v2 파이프라인입니다.
...
```bash
python -m pill_recognition_legacy.cli path/to/image.png --output outputs/result.png
```

폴더 단위 평가:

```bash
python -m pill_recognition_legacy.evaluate_dataset \
...
```bash
python -m pill_recognition_legacy.download_samples --count 60
```

검증셋 2,090장을 모두 받으려면 다음 명령을 사용합니다. 전체 데이터는 수 GB이므로 학습이나 대규모 평가가 필요할 때만 사용합니다.

```bash
python -m pill_recognition_legacy.download_samples --all
```
